...
if (!fileName) {
  console.log(
    "you missed fileName. yarn start -f your_file -p detail_of_number"
  );
  return;
}

parseOBJ(fs.createReadStream(`./assets/${fileName}.obj`), function(
  err,
  result
) {
  if (err) {
...
function updateVertex(result, dataObj){
  ...
  for (let ii = 0; ii < result.facePositions.length; ii++) {
    for (let jj = 0, length = result.facePositions[ii].length - 2; jj < length; jj++) {
      ...
      dataObj.verts.push(
        Number(pos0[0].toFixed(precision)), Number(pos0[1].toFixed(precision)), Number(pos0[2].toFixed(precision)), 
        Number(pos1[0].toFixed(precision)), Number(pos1[1].toFixed(precision)), Number(pos1[2].toFixed(precision)), 
        Number(pos2[0].toFixed(precision)), Number(pos2[1].toFixed(precision)), Number(pos2[2].toFixed(precision))
      );
  ...
  for (let ii = 0; ii < result.faceUVs.length; ii++) {
    for (let jj = 0, length = result.faceUVs[ii].length - 2; jj < length; jj++) {
      ...
      dataObj.texcoords.push(
        Number(pos0[0].toFixed(precision)), Number(pos0[1].toFixed(precision)),  
        Number(pos1[0].toFixed(precision)), Number(pos1[1].toFixed(precision)),  
        Number(pos2[0].toFixed(precision)), Number(pos2[1].toFixed(precision))
      );
...
function writeObjFile(dataObj) {
  // console.log(dataObj.indices.length);
  console.log(dataObj.verts.length);

  const content = JSON.stringify(dataObj);

  fs.writeFile(`./output/${fileName}.json`, content, "utf8", function(err) {
    if (err) {
